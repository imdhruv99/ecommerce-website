...
def getCategories():
    ...
    if isUserAdmin():
        ...
        cur.execute(
            'SELECT category.categoryid, category.category_name, COUNT(product_category.productid)'
            ' as noOfProducts FROM category LEFT JOIN product_category ON '
            'category.categoryid = product_category.categoryid GROUP BY category.categoryid');
    ...
    if isUserAdmin():
        ...
        if form.validate_on_submit():
            ...
            product = Product(sku=form.sku.data, product_name=form.productName.data,
                              ...
                              quantity=form.productQuantity.data, discounted_price=15, product_rating=0,
...
def getUsers():
    ...
    if isUserAdmin():
        ...
        cur.execute(
            'SELECT u.fname, u.lname, u.email, u.active, u.city, u.state, '
            'COUNT(o.orderid) as noOfOrders FROM `user` u LEFT JOIN `order` o ON '
            'u.userid = o.userid GROUP BY u.userid')
...
def seeTrends():
    ...
    if (trendtype == "least"):
        ...
        cur.execute("SELECT ordered_product.productid, sum(ordered_product.quantity) AS "
                    "TotalQuantity,product.product_name FROM \
                       ordered_product,product where ordered_product.productid=product.productid"
                    " GROUP BY productid \
        ...
        cur.execute("SELECT ordered_product.productid, sum(ordered_product.quantity)"
                    " AS TotalQuantity,product.product_name FROM \
                ordered_product,product where ordered_product.productid=product.productid "
                    "GROUP BY productid \
    ...
    return render_template('trends.html', div_placeholder=Markup(my_plot_div), trendtype=trendtype)